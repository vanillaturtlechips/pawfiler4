...
export default function EnsembleRadarChart({ report }: Props) {
  ...
  return (
    ...
    >
      <p className="font-jua text-sm mb-1 text-foreground/80">📊 에이전트 앙상블</p>
      <p className="font-gothic text-xs text-foreground/40 mb-3">각 에이전트의 신뢰도를 종합하여 최종 판정을 도출합니다</p>
      ...
      <div className="mt-3 space-y-1.5">
        <p className="font-gothic text-xs text-foreground/40">가중치 배분</p>
        {weights.map(w => (
          <div key={w.name} className="flex items-center gap-2">
            <span className="font-gothic text-xs text-foreground/50 w-16">{w.name}</span>
            ...
            <span className="font-gothic text-xs text-foreground/40 w-8 text-right">{(w.weight * 100).toFixed(0)}%</span>
          </div>
        ))}
      </div>
    </motion.div>
