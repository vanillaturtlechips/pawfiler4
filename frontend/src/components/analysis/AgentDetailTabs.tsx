...
export default function AgentDetailTabs({ report }: Props) {
  return (
    <Tabs defaultValue="visual" className="w-full">
      <TabsList className="w-full grid grid-cols-4 rounded-2xl h-12" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
        {[
          ...
          <TabsTrigger
            key={t.value}
            value={t.value}
            className="font-jua text-xs rounded-xl data-[state=active]:bg-[rgba(0,137,188,0.2)] data-[state=active]:text-foreground data-[state=active]:shadow-none"
          >
            {t.icon} {t.label}
          </TabsTrigger>
        ))}
            ...
            <>
              ...
              {report.visual.aiModel && (
                <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.03)" }}>
                  <p className="font-jua text-xs mb-2 text-foreground/60">🤖 AI 모델 추정</p>
                  ...
                  <div className="flex gap-2 mt-2 flex-wrap">
                    {report.visual.aiModel.candidates.map(c => (
                      <span key={c.name} className="text-[10px] font-gothic px-2 py-1 rounded-full" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.5)" }}>
              ...
              {report.visual.frames && report.visual.frames.length > 0 && (
                <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.03)" }}>
                  <p className="font-jua text-xs mb-3 text-foreground/60">📊 프레임별 딥페이크 점수</p>
          ...
          {report.audio ? (
            <>
              ...
              {report.audio.segments && report.audio.segments.length > 0 && (
                <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.03)" }}>
                  <p className="font-jua text-xs mb-3 text-foreground/60">🎵 구간별 합성 점수</p>
                  ...
                  <div className="flex justify-between text-[10px] mt-1 text-foreground/30">
                    <span>0초</span>
            ...
            <p className="font-gothic text-xs text-foreground/30 text-center py-8">오디오 분석 데이터 없음</p>
          )}
        </motion.div>
      </TabsContent>
          ...
          {report.llm ? (
            <>
              ...
              <div className="rounded-xl p-4" style={{ background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.12)" }}>
                <p className="font-jua text-xs mb-2 text-foreground/60">💭 Chain of Thought</p>
                <p className="font-gothic text-xs text-foreground/50 leading-relaxed whitespace-pre-wrap">{report.llm.reasoning}</p>
              </div>

              {report.llm.keyFindings.length > 0 && (
                <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.03)" }}>
                  <p className="font-jua text-xs mb-2 text-foreground/60">🔑 핵심 발견</p>
                  <ul className="space-y-1.5">
                    {report.llm.keyFindings.map((f, i) => (
                      <motion.li
                        key={i}
                        className="font-gothic text-xs text-foreground/50 flex items-start gap-2"
            ...
            <p className="font-gothic text-xs text-foreground/30 text-center py-8">LLM 분석 데이터 없음</p>
          )}
        </motion.div>
      </TabsContent>
        ...
        <motion.div className="space-y-4 pt-4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={spring}>
          {report.metadata ? (
            <>
              ...
              <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.03)" }}>
                <p className="font-jua text-xs mb-2 text-foreground/60">📜 인코딩 이력</p>
                <div className="space-y-1">
                  {report.metadata.encodingHistory.map((h, i) => (
                    <div key={i} className="font-gothic text-[10px] text-foreground/40 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: i === 0 ? "#22c55e" : "rgba(255,255,255,0.15)" }} />
                      {h}
                    </div>
                  ))}
                </div>
              ...
              {report.metadata.tamperingIndicators.length > 0 && (
                <div className="rounded-xl p-4" style={{ background: "rgba(220,38,38,0.06)", border: "1px solid rgba(220,38,38,0.12)" }}>
                  <p className="font-jua text-xs mb-2" style={{ color: "#fca5a5" }}>⚠️ 변조 지표</p>
                  <ul className="space-y-1">
                    {report.metadata.tamperingIndicators.map((t, i) => (
                      <li key={i} className="font-gothic text-xs text-foreground/50">• {t}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              {Object.keys(report.metadata.exifData).length > 0 && (
                <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.03)" }}>
                  <p className="font-jua text-xs mb-2 text-foreground/60">📸 EXIF 데이터</p>
                  <div className="space-y-1">
                    {Object.entries(report.metadata.exifData).map(([k, v]) => (
                      <div key={k} className="flex justify-between font-gothic text-[10px]">
            ...
            <p className="font-gothic text-xs text-foreground/30 text-center py-8">메타데이터 분석 데이터 없음</p>
          )}
        </motion.div>
      </TabsContent>
    </Tabs>
...
function InfoRow({ label, value, isBad }: { label: string; value: string; isBad?: boolean }) {
  return (
    <div className="flex items-center justify-between px-4 py-3 rounded-xl" style={{ background: "rgba(255,255,255,0.03)" }}>
      <span className="font-gothic text-xs text-foreground/40">{label}</span>
      <span
...
function MetaChip({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg p-3 text-center" style={{ background: "rgba(255,255,255,0.03)" }}>
      <p className="font-gothic text-[10px] text-foreground/30">{label}</p>
      <p className="font-jua text-sm text-foreground/70 mt-0.5">{value}</p>
    </div>
  );
}
