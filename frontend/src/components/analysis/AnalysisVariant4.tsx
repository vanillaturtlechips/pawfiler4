...
/**
 * Variant 4: Scroll Storytelling
 * 세로 싱글 페이지, 스크롤하면 각 섹션이 fade-in
 */
const AnalysisVariant4 = () => {
  const a = useAnalysis();
    ...
    <div ref={containerRef} className="w-full overflow-y-auto" style={{ minHeight: "calc(100vh - 5rem)" }}>
      ...
      <motion.section className="flex flex-col items-center px-4 pb-16" {...fadeInUp}>
        <div className="w-full max-w-lg">
          ...
          <GameButton
            variant="blue"
            className={`text-xl ${!a.selectedFile || a.isAnalyzing ? "opacity-40 pointer-events-none" : ""}`}
            onClick={a.handleAnalyze}
          >
            {a.isAnalyzing ? "⏳ 구슬이 분석하는 중..." : "🔮 마법 구슬아, 분석해줘!"}
          </GameButton>
...
export default AnalysisVariant4;
