...
import { 
  ...
  Edit2, Save, X, BookOpen, BarChart3, Settings, FileText, Loader2
} from "lucide-react";
import { generateReport, downloadReport, getUserId } from "@/lib/api";
...
const ProfilePage = () => {
  ...
  const handleGenerateReport = async () => {
    ...
    setReportError(null);
    try {
      const { report_url } = await generateReport(reportDays ?? undefined);
      // S3 presigned URL이면 바로 열고, 로컬 경로면 기존 방식으로 다운로드
      if (report_url.startsWith("http")) {
        window.open(report_url, "_blank");
      } else {
        downloadReport(getUserId());
      }
    } catch (e: any) {
      setReportError(e.message || "리포트 생성에 실패했어요.");
    } finally {
            ...
            <ParchmentPanel className="rounded-2xl border-4 p-4 flex-1 min-h-0 overflow-hidden shadow-xl">
              ...
              {activeTab === "settings" && (
                <div className="h-full overflow-y-auto">
                  ...
                  {/* Report Download */}
                  <div className="p-3 rounded-lg bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-300 mb-3">
                    <div className="flex items-center gap-2 mb-2">
                      <FileText className="w-5 h-5 text-blue-600" />
                      <h4 className="font-jua text-base text-blue-900">나의 분석 리포트</h4>
                    </div>
                    <p className="text-xs text-wood-dark mb-2">
                      선택한 기간의 퀴즈 데이터를 기반으로 오답 히트맵과 개선 포인트를 담은 리포트를 생성해요.
                    </p>
                    <Select
                      value={reportDays === null ? "all" : String(reportDays)}
                      onValueChange={(v) => setReportDays(v === "all" ? null : Number(v))}
                    >
                      <SelectTrigger className="w-full mb-2 font-jua text-sm border-blue-300 bg-white">
                        <SelectValue placeholder="기간 선택" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="7" className="font-jua">최근 7일</SelectItem>
                        <SelectItem value="30" className="font-jua">최근 30일</SelectItem>
                        <SelectItem value="90" className="font-jua">최근 90일</SelectItem>
                        <SelectItem value="all" className="font-jua">전체 기간</SelectItem>
                      </SelectContent>
                    </Select>
                    {reportError && (
                    ...
                    <Button
                      ...
                      className="w-full font-jua text-sm py-2 bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white rounded-lg"
                    >
                      {reportLoading
                        ? <><Loader2 className="w-4 h-4 mr-1 animate-spin" />생성 중...</>
                        : <><FileText className="w-4 h-4 mr-1" />리포트 다운로드</>
                      }
                    </Button>
                  </div>

                  {/* Premium Upgrade - Compact */}
                  {user.subscriptionType === "free" && (                    <div className="p-3 rounded-lg bg-gradient-to-br from-yellow-50 to-orange-50 border border-orange-300">
                      <div className="flex items-center gap-2 mb-2">
